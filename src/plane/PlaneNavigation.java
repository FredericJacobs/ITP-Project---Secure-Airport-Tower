...
public class PlaneNavigation implements Runnable {
	...
	private void moveInStraightLine(double deltaTimeSeconds, RoutingInstruction instruction) {
	...
	private void moveInCircle(double deltaTimeSeconds, RoutingInstruction instruction) {
		...
		double speed = planeSpeed/12;
		double instructionAngle = Math.toRadians(instruction.getAngle()); // Angle en radians
		double instructionAngleSign = Math.signum(instructionAngle); // Signe de l'angle
		...
		if (deltaTheta < Math.abs(instructionAngle)) {
			...
			modY = (r * Math.sin(theta) + instruction.getyCoord());
			Plane.setPosition(new XYPosition((int)modX, (int)modY));
			System.out.println("Plane " + TestPlane.getPlaneID().toString() + " arrived at waypoint (" + modX + ", " + modY + ").");
			if (deltaTheta > Math.abs(instructionAngle)) {
