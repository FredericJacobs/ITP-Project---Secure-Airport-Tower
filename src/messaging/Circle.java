...
public class Circle {
	public static void answerLandingRequest(Plane plane, DataOutputStream outData)
		...
		if (Tower.landingRoute.size() == 0) {
			System.out.println("Tower tries to give a direct landing instruction");
			...
			/*	RoutingMessage respondLanding = new RoutingMessage(
					...
					moveType.CIRCULAR, int2bytes(0));
			respondLanding.write(outData);*/
			RoutingMessage respondLanding1 = new RoutingMessage(
					"Tour0000".getBytes(), Tower.straightX,
					Tower.straightY, routingMessageType.REPLACEALL,
					moveType.STRAIGHT, int2bytes(0));
			...
			System.out.println("Tower tries to give a small circle to the seconde landing plane");
