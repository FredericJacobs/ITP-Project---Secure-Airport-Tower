...
public class AirportPanel extends JPanel {
	...
	public AirportPanel() {
		...
		try {
			this.imgBack = ImageIO.read(new File("src" + File.separator + "GUI"
					+ File.separator + "img" + File.separator + "map.png"));
			this.imgPlane = ImageIO.read(new File("src" + File.separator
					+ "GUI" + File.separator + "img" + File.separator
					+ "plane.png"));
			this.imgTower = ImageIO.read(new File("src" + File.separator
					+ "GUI" + File.separator + "img" + File.separator
					+ "tower.png"));
			this.imgKboom = ImageIO.read(new File("src" + File.separator
					+ "GUI" + File.separator + "img" + File.separator
					+ "kboom.png"));
			this.imgRadar = ImageIO.read(new File("src" + File.separator
					+ "GUI" + File.separator + "img" + File.separator
					+ "radar.png"));
			this.imgPlaneMayDay = ImageIO.read(new File("src" + File.separator
					+ "GUI" + File.separator + "img" + File.separator
					+ "plane_mayday.png"));
		} catch (IOException e) {
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < Tower.getInstance().getJournal().positions.size(); i++) {
			XYPosition newPosition = Tower.getInstance().getJournal().positions
					.get(i).getPosition();
			Point p = new Point(newPosition.getPosx(), newPosition.getPosy());
			String planeId = new String(
		...
		for (int i = 0; i < Tower.getInstance().getJournal().positions.size(); i++) {
			...
			String planeId = new String(
					Tower.getInstance().getJournal().positions.get(i)
							.getPlaneID());
			BufferedImage planeImg = Tower.getInstance().getJournal().positions
					.get(i).hasCrashed() ? imgKboom : imgPlane;
			...
			XYPosition newPosition = Tower.getInstance().getJournal().positions
					.get(i).getPosition();
			g2d.setTransform(AffineTransform.getRotateInstance(0,
					newPosition.getPosx(), newPosition.getPosy()));
			g2d.drawString(planeId + " (" + newPosition.getPosx() + ", "
					+ newPosition.getPosy() + ")", newPosition.getPosx()
					+ planeImg.getWidth() / 2, newPosition.getPosy());

			// Compute the rotation angle of the plane, and draw it
			CircularBuffer<Point> previousPos = previousPositions.get(planeId);
			double dx = previousPos.get(previousPos.size() - 1).getX()
					- previousPos.get(previousPos.size() - 2).getX();
			double dy = previousPos.get(previousPos.size() - 1).getY()
					- previousPos.get(previousPos.size() - 2).getY();
			double theta = Math.atan2(dy, dx);
			g2d.setTransform(AffineTransform.getRotateInstance(theta,
					newPosition.getPosx(), newPosition.getPosy()));
			g2d.drawImage(planeImg, newPosition.getPosx() - planeImg.getWidth()
					/ 2, newPosition.getPosy() - planeImg.getHeight() / 2, null);
		}
	}
